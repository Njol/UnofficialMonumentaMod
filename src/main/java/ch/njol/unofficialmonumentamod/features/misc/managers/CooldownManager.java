...
public class CooldownManager {
    //TODO add a use case to get the cooldown logic for tesseract of light, or not -> because WHY does it have to work differently than all of the other tesseracts combined.
    private static final MinecraftClient mc = MinecraftClient.getInstance();
    private static final HashMap<Item, CooldownEntry> entries = new HashMap<>();
    private static int tick;

    private static final Pattern COOLDOWN_PATTERN = Pattern.compile("^Cooldown : (?<cooldown>[0-9]{0,3})(?<unit>[sm])(?<post>.*)");
    ...
    public static void removeCooldownFromItem(ItemStack item, Trigger trigger) {
        addCooldownToItem(item, null, trigger);
    }
